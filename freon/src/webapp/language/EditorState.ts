...
export class EditorState {
     ...
     * @param modelName
     */
    async openLionWebDefaultModel(modelName: string) {
        LOGGER.log("EditorState.openLionwebModel(" + modelName + ")");
        editorProgressShown.set(true);
        this.resetGlobalVariables();
        ...
        currentModelName.set(modelName);
        // fill the new model with the units loaded from the server
        await serverCommunication.loadModelUnit(modelName, modelName, (unit: FreModelUnit) => {
            console.log("Reading unit " +unit.name + ": " + unit.freLanguageConcept());
            unitNames.set([unit.name]);
            EditorState.getInstance().currentModel.addUnit(unit);
            EditorState.getInstance().currentUnit = unit;
        });
        console.log("open unit " + EditorState.getInstance().currentUnit.name);
        currentUnitName.set(this.currentUnit.name);
        EditorState.getInstance().showUnitAndErrors(this.currentUnit);
    }

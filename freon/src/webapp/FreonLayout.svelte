<script lang="ts">
	...
	import { idProvider, serverCommunication } from "./config/WebappConfiguration";
	...
	onMount(async () => {
		...
		// TODO Hack: where should this go?
		idProvider.availableIds = await serverCommunication.generateIds(100, null);
		const urlParams = new URLSearchParams(window.location.search);
		const model = urlParams.get('model');
		// TODO Shortcut: open the default lionweb model if there is a model parameter
		if (model !== null) {
			openModel(model);
		} else {
	...
	function openModel(model: string) {
		const comm = EditorState.getInstance();
		// if(model === "the-form") {
			comm.openLionWebDefaultModel(model);
		// } else {

...
import { PropertiesInterpreterInit } from "./gen/PropertiesInterpreterInit";
 ...
 */
export class MainPropertiesInterpreter implements FreInterpreter {
    private static main: IMainInterpreter = null;

    constructor() {
        if (MainPropertiesInterpreter.main === null) {
            MainPropertiesInterpreter.main = MainInterpreter.instance(PropertiesInterpreterInit, getConceptFunction, getPropertyFunction);
        }
    }

    setTracing(value: boolean) {
        MainPropertiesInterpreter.main.setTracing(value);
    }

    getTrace(): InterpreterTracer {
        return MainPropertiesInterpreter.main.getTrace();
    }

    evaluate(node: Object): RtObject {
        MainPropertiesInterpreter.main.reset();
        try {
            return MainPropertiesInterpreter.main.evaluate(node, InterpreterContext.EMPTY_CONTEXT);
        } catch (e: any) {

...
import { ItemGroup } from "../../language/gen";

/**
 * This class implements the box provider for a single node of type ItemGroup.
 ...
 */
export class ItemGroupBoxProvider extends FreBoxProvider {
    constructor(mainHandler: FreProjectionHandler) {
        super(mainHandler);
        this.knownBoxProjections = ["default"];
        this.knownTableProjections = ["default"];
        this.conceptName = "ItemGroup";
    }
    ...
    private getDefault(): Box {
        return BoxFactory.verticalLayout(this._element as ItemGroup, "ItemGroup-overall", "", [
            BoxFactory.horizontalLayout(
                this._element as ItemGroup,
                "ItemGroup-hlist-line-0",
                "",
                [
                    BoxUtil.labelBox(this._element as ItemGroup, "Itemgroup", "top-1-line-0-item-0"),
                    BoxUtil.textBox(this._element as ItemGroup, "name")
                ],
                { selectable: false }
            ),
            BoxUtil.labelBox(this._element as ItemGroup, "Items:", "top-1-line-1-item-0"),
            BoxUtil.indentBox(
                this._element as ItemGroup,
                4,
                "2",
                BoxUtil.verticalPartListBox(this._element as ItemGroup, (this._element as ItemGroup).items, "items", null, this.mainHandler)
            ),
            BoxUtil.emptyLineBox(this._element as ItemGroup, "ItemGroup-empty-line-3")
        ]);
    }
}

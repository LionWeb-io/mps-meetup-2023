...
import { PropertiesFileSyntaxAnalyserPart, PropertiesCommonSyntaxAnalyserPart } from ".";

/**
 *   Class PropertiesSyntaxAnalyser is the main syntax analyser.
 ...
 *
 */
export class PropertiesSyntaxAnalyser implements SyntaxAnalyser {
    sourceName: string = "";
    locationMap: any;
    private _unit_PropertiesFile_analyser: PropertiesFileSyntaxAnalyserPart = new PropertiesFileSyntaxAnalyserPart(this);
    private _unit_common_analyser: PropertiesCommonSyntaxAnalyserPart = new PropertiesCommonSyntaxAnalyserPart(this);
    ...
    public transformSharedPackedParseTreeNode(node: SPPTNode): any {
        if (!!node) {
            try {
                ...
                if (e.message.startsWith("Syntax error in ") || e.message.startsWith("Error in PropertiesSyntaxAnalyser")) {
                    throw e;
                } else {
    ...
    private transformSharedPackedParseTreeBranch(branch: SPPTBranch): any {
        ...
        if ("PropertiesFile" === brName) {
            ...
            return this._unit_PropertiesFile_analyser.transformDecValue(branch);
        } else if ("IntValue" === brName) {
            ...
            throw new Error(`Error in PropertiesSyntaxAnalyser: ${brName} not handled for node '${branch?.matchedText}'`);
        }
    }

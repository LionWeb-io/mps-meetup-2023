...
import { FormSyntaxAnalyserPart, SimpleformsCommonSyntaxAnalyserPart } from ".";

/**
 *   Class SimpleformsSyntaxAnalyser is the main syntax analyser.
 ...
 *
 */
export class SimpleformsSyntaxAnalyser implements SyntaxAnalyser {
    sourceName: string = "";
    locationMap: any;
    private _unit_Form_analyser: FormSyntaxAnalyserPart = new FormSyntaxAnalyserPart(this);
    private _unit_common_analyser: SimpleformsCommonSyntaxAnalyserPart = new SimpleformsCommonSyntaxAnalyserPart(this);
    ...
    public transformSharedPackedParseTreeNode(node: SPPTNode): any {
        if (!!node) {
            try {
                ...
                if (e.message.startsWith("Syntax error in ") || e.message.startsWith("Error in SimpleformsSyntaxAnalyser")) {
                    throw e;
                } else {
    ...
    private transformSharedPackedParseTreeBranch(branch: SPPTBranch): any {
        ...
        if ("Form" == brName) {
            return this._unit_Form_analyser.transformForm(branch);
        } else if ("ItemGroup" == brName) {
            return this._unit_Form_analyser.transformItemGroup(branch);
        } else if ("SimpleItem" == brName) {
            return this._unit_Form_analyser.transformSimpleItem(branch);
        } else if ("StringType" == brName) {
            return this._unit_Form_analyser.transformStringType(branch);
        } else if ("IntType" == brName) {
            return this._unit_Form_analyser.transformIntType(branch);
        } else if ("Type" == brName) {
            return this._unit_Form_analyser.transformType(branch);
        } else if ("__fre_reference" == brName) {
            return this.transform__fre_reference(branch);
        } else {
            throw new Error(`Error in SimpleformsSyntaxAnalyser: ${brName} not handled for node '${branch?.matchedText}'`);
        }
    }

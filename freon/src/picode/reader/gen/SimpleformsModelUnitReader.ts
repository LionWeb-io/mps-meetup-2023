...
import { SimpleformsModelUnitType, ModelUnitMetaType, MainModel } from "../../language/gen";
import { SimpleformsGrammarStr } from "./SimpleformsGrammar";
import { SimpleformsSyntaxAnalyser } from "./SimpleformsSyntaxAnalyser";
import { SimpleformsSemanticAnalyser } from "./SimpleformsSemanticAnalyser";

/**
 *   Class SimpleformsModelUnitReader is a wrapper for the various parsers of
 *   modelunits.
 */
export class SimpleformsModelUnitReader implements FreReader {
    analyser: SimpleformsSyntaxAnalyser = new SimpleformsSyntaxAnalyser();
    parser: LanguageProcessor = Agl.processorFromString(SimpleformsGrammarStr, this.analyser, null, null);
     ...
     */
    readFromString(sentence: string, metatype: ModelUnitMetaType, model: MainModel, sourceName?: string): SimpleformsModelUnitType {
        ...
        // parse the input
        let unit: SimpleformsModelUnitType = null;
        if (this.parser) {
            try {
                let asm;
                ...
                unit = asm as SimpleformsModelUnitType;
            } catch (e) {
            ...
            if (!!model) {
                try {
                    if (model.getUnits().filter(existing => existing.name === unit.name).length > 0) {
                        ...
                        model.addUnit(unit);
                        const semAnalyser = new SimpleformsSemanticAnalyser();

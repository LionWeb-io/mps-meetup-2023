...
import { PropertiesModelUnitType, ModelUnitMetaType, MainModel } from "../../language/gen";
import { PropertiesGrammarStr } from "./PropertiesGrammar";
import { PropertiesSyntaxAnalyser } from "./PropertiesSyntaxAnalyser";
import { PropertiesSemanticAnalyser } from "./PropertiesSemanticAnalyser";

/**
 *   Class PropertiesModelUnitReader is a wrapper for the various parsers of
 *   modelunits.
 */
export class PropertiesModelUnitReader implements FreReader {
    analyser: PropertiesSyntaxAnalyser = new PropertiesSyntaxAnalyser();
    parser: LanguageProcessor = Agl.processorFromString(PropertiesGrammarStr, this.analyser, null, null);
     ...
     */
    readFromString(sentence: string, metatype: ModelUnitMetaType, model: MainModel, sourceName?: string): PropertiesModelUnitType {
        this.analyser.sourceName = sourceName;
        let startRule: string = "";
        // choose the correct parser
        if (metatype === "PropertiesFile") {
            startRule = "PropertiesFile";
        }

        // parse the input
        let unit: PropertiesModelUnitType = null;
        if (this.parser) {
            try {
                let asm;
                ...
                unit = asm as PropertiesModelUnitType;
            } catch (e) {
            ...
            if (!!model) {
                try {
                    if (model.getUnits().filter(existing => existing.name === unit.name).length > 0) {
                        ...
                        model.addUnit(unit);
                        const semAnalyser = new PropertiesSemanticAnalyser();
